#Yield Curves Code
#' @export
getYieldCurve <- function(params, species, F_range, F_max = 1, F_min = 0,
                        no_steps = 10, distance_func = distanceSSLogN,
                        tol = 0.001, t_max = 100, ...)
    UseMethod("getYieldCurve")

#' @export
getYieldCurve.MizerParams <- function(params,
                                    species,
                                    F_range,
                                    F_max = 1,
                                    F_min = 0,
    ...
    # --- NEW: Calculate catchability-weighted selectivity profile ---
    gears_to_weight <- gp$gear[sp_sel]
    catchabilities <- gp$catchability[sp_sel]
    total_catchability <- sum(catchabilities)

    if (total_catchability == 0) {
        weights <- rep(1 / length(catchabilities), length(catchabilities))
    } else {
        weights <- catchabilities / total_catchability
    }

    # Extract the 3D selectivity array: [gear, species, size]
    old_sel <- selectivity(params)[gears_to_weight, sp_name, , drop = FALSE]
    num_sizes <- dim(old_sel)[3]
    weighted_sel_vec <- numeric(num_sizes)

    for (i in seq_along(gears_to_weight)) {
        weighted_sel_vec <- weighted_sel_vec + weights[i] * old_sel[i, 1, ]
    }
    # -----------------------------------------------------------------

    # Base the new gear on the first gear that catches this species for default params
    ...
    # --- NEW: Overwrite the tmp gear's selectivity with our weighted average ---
    params@selectivity["tmp", sp_name, ] <- weighted_sel_vec
...
#' @export
plotYieldCurve <- function(params, species, F_range, F_max = 1, F_min = 0,
                         no_steps = 10, distance_func = distanceSSLogN,
                         tol = 0.001, t_max = 100, ...)
    UseMethod("plotYieldCurve")

#' @export
plotYieldCurve.MizerParams <- function(params,
                                     species,
                                     F_range,
                                     F_max = 1,
                                     F_min = 0,
...
yieldCalculator.MizerParams <- function(params, effort_vec, idx_species,
    ...
    for (i in seq_along(effort_vec)) {
        ...
        if (ft < t_max - 1) { # if convergence use final yield
            ...
            # --- FIX FOR OLDER MIZER VERSIONS ---
            # Replace: params <- initialParams(sim)
            params <- sim@params
            # ------------------------------------
            ...
            # --- FIX FOR OLDER MIZER VERSIONS ---
            # Replace: params <- initialParams(sim, time_range = c(65, 100))
            # Older versions don't filter initialParams by time_range easily here,
            # so we fall back safely to the simulation's base params object.
            params <- sim@params
            # ------------------------------------
        }
    }
